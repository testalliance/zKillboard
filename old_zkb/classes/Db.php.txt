...
class Db
{
     ...
     */
    public static function query($query, $params = array(), $cacheTime = 120)
    {
		...
		if ($cacheTime != 0 && $cacheTime < 300) $cacheTime = 300;
        ...
        if ($cacheTime > 0) {
            Bin::set($key, $result);
            if (!Memcached::set($key, $result, min(3600, $cacheTime))) die('memcache failure');
        }
        if ($duration > 100) Db::log($query, $params, $duration);
        return $result;
    }

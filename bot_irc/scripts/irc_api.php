	...
	public function execute($nick, $uhost, $channel, $command, $parameters, $nickAccessLevel) {
		...
		switch ($entity) {
			...
			case "stats":
				...
				return "API Stats 1 Hour:|g|$sum |n|requests with |r|$errorSum |n|errors. Frequency:|g|$sumFreq|n|/s |r|$errorFreq|n|/s";
		}
		...
		if (sizeof($keyIDs) == 0) return "|r|Unable to locate any keys associated with $entity |n|";

		foreach($keyIDs as $keyID) {
			if ($keyID == 0) return "Please provide a valid keyID.";
			$info = Db::queryRow("select * from zz_api where keyID = :k", array(":k" => $keyID), 0);
			if ($info == null) return "Could not find KeyID $keyID";
			...
			if (sizeof($detail) == 0) {
				$hasErrors = true;
				return "KeyID $keyID has |r|no characters|n| assigned to it.";
			}
			foreach($detail as $row) {
				...
				return "KeyID |g|$keyID|n| | Type: $type | Error Code: $errorCode | Last Checked: $lastChecked | Cached Until: $cachedUntil $details";
			}
			...
			if ($hasErrors) {
				...
				return "KeyID Overview $keyID|n| | $vCodeShort | Error Code: $errorCode | Last Validated: $lastValidated | Added: $dateAdded";
			}
		}
	}
